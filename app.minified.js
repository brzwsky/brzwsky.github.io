	...
	setupMostVisibleHighlighting() {
		const collectCards = () => {
			const cards = Array.from(
				document.querySelectorAll(
					'.casino-card, .casino-card--top, .casino-card--top',
				),
			).filter(Boolean);
		...
		const applyMostVisible = () => {
			...
			if (maxEl !== mostVisibleCard) {
				if (mostVisibleCard) {
					mostVisibleCard.classList.remove('is-most-visible');
					mostVisibleCard.classList.remove('is-primary-most-visible');
				}
				mostVisibleCard = maxEl;
				if (mostVisibleCard) mostVisibleCard.classList.add('is-most-visible');
			}
		};
...
class ScrollRevealManager {
	constructor() {
		...
		this.sectionElements = {
			faq: this.faqSection
				? Array.from(this.faqSection.querySelectorAll('.u-fade-in'))
				: [],
			casino: this.casinoSection
				? Array.from(this.casinoSection.querySelectorAll('.u-fade-in'))
				: [],
			hero: this.heroSection
				? Array.from(this.heroSection.querySelectorAll('.u-fade-in'))
				: [],
			other: [],
		};
	...
	updateToggleButtonState(isDark) {
		if (this.toggleButton) {
			if (isDark) {
				...
				this.toggleButton.classList.add('is-dark-mode');
...
const GEO_DEV_HOSTNAMES = new Set(['localhost', '127.0.0.1']);
const GEO_UNKNOWN_VALUES = new Set(['UNKNOWN', 'XX']);
function normalizeCountry(code) {
	if (!code) return null;
	const normalized = String(code).trim().toUpperCase();
	if (!/^[A-Z]{2}$/.test(normalized)) return null;
	if (GEO_UNKNOWN_VALUES.has(normalized)) return null;
	return normalized;
}
function isGeoDevHost() {
	return GEO_DEV_HOSTNAMES.has(window.location.hostname);
}
function getGeoOverrideFromQuery() {
	if (!isGeoDevHost()) return null;
	const params = new URLSearchParams(window.location.search);
	const override = params.get('geo');
	return normalizeCountry(override);
}
function getVisitorCountry() {
	const devOverride = getGeoOverrideFromQuery();
	if (devOverride) {
		if (isGeoDevHost()) {
			console.info('[GEO][DEV] Override country via ?geo= ->', devOverride);
		}
		return devOverride;
	}
	const meta = document.querySelector('meta[name="visitor-country"]');
	if (!meta) {
		if (isGeoDevHost()) {
			console.warn('[GEO][DEV] meta[name="visitor-country"] not found');
		}
		return null;
	}
	const normalized = normalizeCountry(meta.getAttribute('content'));
	if (isGeoDevHost()) {
		console.info('[GEO][DEV] Meta country ->', normalized || 'null');
	}
	return normalized;
}
function getCurrentLocaleCountry() {
	return window.location.pathname.startsWith('/ca/') ? 'CA' : 'FR';
}
class GeoPopup {
	constructor() {
		this.popupId = 'geo-popup-ca';
		this.localStorageKey = 'topjeu_geo_popup_shown';
		this.init();
	}
	init() {
		const visitorCountry = getVisitorCountry();
		if (!visitorCountry) return;
		const currentLocaleCountry = getCurrentLocaleCountry();
		if (visitorCountry === currentLocaleCountry) return;
		let targetLocale = null;
		if (visitorCountry === 'CA' && currentLocaleCountry === 'FR') {
			targetLocale = 'CA';
		} else if (visitorCountry === 'FR' && currentLocaleCountry === 'CA') {
			targetLocale = 'FR';
		} else {
			if (isGeoDevHost()) {
				console.info('[GEO][DEV] No target locale match:', {
					visitorCountry,
		...
		const storageKey = `${this.localStorageKey}_${targetLocale}`;
		const alreadyShown = localStorage.getItem(storageKey);
		if (!alreadyShown) {
			if (isGeoDevHost()) {
				console.info('[GEO][DEV] Show popup:', {
					visitorCountry,
					currentLocaleCountry,
					targetLocale,
					storageKey,
			...
			this.showPopup({ targetLocale, storageKey });
		} else if (isGeoDevHost()) {
			console.info('[GEO][DEV] Popup already shown:', storageKey);
		}
	}
	showPopup({ targetLocale, storageKey }) {
		const isTargetCanada = targetLocale === 'CA';
		const title = isTargetCanada ? '🇨🇦 TopJeu Canada' : '🇫🇷 TopJeu France';
		const message = isTargetCanada
			? 'Bienvenue au Canada ! Découvrez notre version spécialement adaptée pour les joueurs canadiens.'
			: 'Vous consultez la version Canada. Souhaitez-vous revenir sur la version France ?';
		const ctaHref = isTargetCanada ? '/ca/' : '/';
		const ctaText = isTargetCanada
			? 'Visiter TopJeu Canada'
			: 'Aller sur TopJeu France';
		...
		overlay.innerHTML = `<div class="geo-popup-content"><button class="geo-popup-close"aria-label="Fermer">✕</button><h2 id="geo-popup-title">${title}</h2><p>${message}</p><div class="geo-popup-actions"><a href="${ctaHref}"class="geo-popup-cta">${ctaText}</a><button class="geo-popup-dismiss">Plus tard</button></div></div>`;
		document.body.appendChild(overlay);
		requestAnimationFrame(() => {
			overlay.classList.add('is-visible');
		});
		const closeBtn = overlay.querySelector('.geo-popup-close');
		const dismissBtn = overlay.querySelector('.geo-popup-dismiss');
		const ctaLink = overlay.querySelector('.geo-popup-cta');
		const closePopup = () => {
			overlay.classList.remove('is-visible');
			setTimeout(() => overlay.remove(), 300);
			localStorage.setItem(storageKey, 'true');
			if (isGeoDevHost()) {
				console.info('[GEO][DEV] Popup closed, set storage:', storageKey);
			}
		};
		...
		ctaLink.addEventListener('click', () => {
			localStorage.setItem(storageKey, 'true');
			if (isGeoDevHost()) {
				console.info('[GEO][DEV] CTA click, set storage:', storageKey);
			}
		});
...
document.addEventListener('DOMContentLoaded', () => {
	const fadeIns = Array.from(document.querySelectorAll('.u-fade-in'));
	fadeIns.forEach((el) => {
